...
def iniGeom(mesh) :
  ...
  mshtools.mmg3d(mesh,0,None,path.HMIN,path.HMAX,path.HAUSD,path.HGRAD,0,mesh)
  ...
  # Call to mmg3d for discretizing the level set function into the mesh
  mshtools.mmg3d(mesh,1,path.TMPSOL,path.HMIN,path.HMAX,path.HAUSD,path.HGRAD,1,mesh)
